...
import { ArrowLeft, Plus, X, Search, Loader2, Check } from 'lucide-react';
...
export default function NewRecipePage() {
  ...
  const nutrientSummary = useMemo(() => {
    ...
    // Sum ingredient nutrients then divide by servings
  ...
  const handleSubmit = async () => {
    ...
    const input: Parameters<typeof createRecipe.mutateAsync>[0] = {
      name: name.trim(),
    ...
    try {
      ...
      // error handled via createRecipe.error
    }
  };
    ...
    return (
      <main className="mx-auto max-w-lg px-4 py-6 text-center">
        <div className="mt-16 space-y-3">
          ...
          <p className="text-lg font-semibold text-white">Recipe Created!</p>
  ...
  return (
    <main className="mx-auto max-w-lg px-4 py-6">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-white">New Recipe</h1>
      </div>

      {/* Error */}
      {createRecipe.error && (
        <div className="mb-4 rounded-lg border border-red-700 bg-red-900/30 px-4 py-3 text-sm text-red-300">
          {(createRecipe.error as Error).message || 'Failed to create recipe. Please try again.'}
        </div>
      )}
