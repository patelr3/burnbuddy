...
- The StreakDots component currently uses a horizontal layout (label + dots side-by-side). To fit in a single grid column, switch to a stacked layout: first row is `🔥 {count} {label}`, second row is the 7 dots (no day labels)
- The dots currently use `gap-1.5` (6px) on web and `gap: 6` on mobile — with day labels removed, the dots should fit comfortably in ~50% width
- Day-of-week labels (S, M, T, W, T, F, S) are removed from both web and mobile to save horizontal space
